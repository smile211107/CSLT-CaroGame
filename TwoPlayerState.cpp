...
void TwoPlayerState::handleEvent(const sf::Event& event) {
    ...
    if (const auto* key = event.getIf<sf::Event::KeyPressed>()) {
        ...
        if (key->scancode == sf::Keyboard::Scancode::Up) {
            if (menuInputs.empty()) return;
            selectedButtonIndex = (selectedButtonIndex - 1 + static_cast<int>(menuInputs.size())) % static_cast<int>(menuInputs.size());
            updateActiveInput();
        }
        else if (key->scancode == sf::Keyboard::Scancode::Down) {
            if (menuInputs.empty()) return;
            selectedButtonIndex = (selectedButtonIndex + 1) % static_cast<int>(menuInputs.size());
            updateActiveInput();
        }
