    ...
    )
{
    ...
    gameMode = 2;
    menuInputs.push_back({ &user1,1 });
    menuInputs.push_back({&user2, 2});
...
void TwoPlayerState::updateActiveInput() {
    ...
    if (selectedButtonIndex == menuInputs.size()) {
    ...
    PlayerNameInput* input = menuInputs[selectedButtonIndex].first;
...
void TwoPlayerState::handleEvent(const sf::Event& event) {
    ...
    if (const auto* textEvent = event.getIf<sf::Event::TextEntered>())
    {
        ...
        if (!menuInputs.empty()) {
            menuInputs[selectedButtonIndex].first->type(textEvent->unicode);
        }
    }
    ...
    if (const auto* key = event.getIf<sf::Event::KeyPressed>()) {
        ...
        if (key->scancode == sf::Keyboard::Scancode::Up) {
            ...
            if (menuInputs.empty()) return;
            selectedButtonIndex = (selectedButtonIndex + 1) % (menuInputs.size() + 1);
            updateActiveInput();
        }
            ...
            PlayerNameInput* currentInput = menuInputs[selectedButtonIndex].first;
            std::string confirmedName = currentInput->getName();


            playerNames[menuInputs[selectedButtonIndex].second] = confirmedName;
        ...
        if (mouseEvent->button == sf::Mouse::Button::Left) {
            ...
            if (buttonBack->isMouseOver(mousePos)) {
                ...
                for (int i = 0; i < menuInputs.size(); ++i) {
                    if (menuInputs[i].first->checkClick(mousePos)) {
